...
{const basic=T.create('wand','hard'),advanced=T.create('wand','expert');assert.ok(T.roomSpec(basic).count<T.roomSpec(advanced).count);assert.ok(T.roomSpec(basic).speedScale<T.roomSpec(advanced).speedScale);assert.ok(T.roomSpec(basic).hpScale<T.roomSpec(advanced).hpScale);assert.ok(T.roomSpec(basic).damageScale<T.roomSpec(advanced).damageScale);
 ...
 for(const [id,key,direction] of [['ember','damage',1],['heart','maxHp',1],['frost','armor',-1],['storm','interval',-1],['nova','skillDamage',1]]){const before=T.stats(basic)[key];for(const cost of [20,35,50]){const cash=basic.crystals;if(cash<cost)basic.crystals=cost;const old=basic.crystals;assert.equal(T.upgradeCost(basic,id),cost);assert.ok(T.upgrade(basic,id));assert.equal(basic.crystals,old-cost);}assert.ok((T.stats(basic)[key]-before)*direction>0);const cash=basic.crystals;assert.ok(!T.upgrade(basic,id));assert.equal(basic.crystals,cash);}
