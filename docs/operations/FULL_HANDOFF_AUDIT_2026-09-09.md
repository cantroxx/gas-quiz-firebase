...
다만 “모든 보완이 끝난 무결점 상태”는 아니다. 새로 확인된 Functions 운영 종속성 취약점, 의도적으로 아직 배포하지 않은 퀴즈타운 두 변경, 작은 공개 콘솔 오류 두 건, Rules 행동 테스트 부재가 후속 우선순위다. 문화유산 게임은 다학교 자율 가입 방식까지 운영 반영됐으나 기존 데이터 경로의 과거 공개 위험은 호환 기간 동안 남아 있다. 어떤 항목도 이번 점검에서 학생 데이터를 변경하지 않았다.
...
단, Git 밖 상위 `/Users/kdw/Projects/AGENTS.md`의 “현재 특별 주의” 두 줄은 최신 상태를 따라오지 못했다. `life-diary`를 아직 원격 없음으로, 문화유산을 아직 UID 전환 전으로 설명한다. 각 프로젝트 코드나 배포가 뒤로 돌아간 것은 아니며, 상위 공통 문서의 상태 요약만 오래된 것이다. 현재 쓰기 범위 밖 파일이므로 이번 점검에서는 수정하지 않았고 이 보고서를 우선 기준으로 둔다. 상위 폴더를 작업공간으로 열 때 공통 `AGENTS.md`를 동기화해야 한다.

## 2. Git·원격 보존 상태

점검 시작 시 아래 15개 저장소는 모두 `main` 작업 트리가 깨끗했고, 실제 원격 `refs/heads/main`과 로컬 HEAD가 일치했다. 원격에만 있거나 로컬에만 남은 커밋은 없었다. 현재 `gas-quiz-firebase`에는 이 점검으로 갱신한 문서만 미커밋 변경으로 남아 있다.
...
중앙 문서에 남아 있던 `8786b01`과 “교사가 전환 코드를 배부”하는 설명은 최신이 아니었다. 실제 최신 `seoul-heritage-main`은 `79eeaca`이며 GitHub Pages 배포본과 일치한다.
...
- PIN 분실 복구 정책과 Kakao 허용 도메인은 계속 결정·확인이 필요
