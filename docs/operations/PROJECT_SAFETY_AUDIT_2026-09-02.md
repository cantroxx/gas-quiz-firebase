...
2. 출처가 복구되지 않은 `dream-class` 아바타와 낱말대전 사전의 운영 반영
...
| `wordbattle` | 🟡 로컬 고지 완료·운영 반영 대기 | Claude 기록에서 국립국어원 XLS, `hunspell-dict-ko` 0.7.94, 입력 해시와 생성 로직을 복구했다. 기존 80,774개 본문을 재현했고 GPL-3.0 전문·화면 링크를 로컬에 포함했다 | 사전 본문은 유지하고 사전만 떼어 별도 재배포하지 않음. 운영 반영은 사용자 승인 후 수행 |
...
`public/wordbattle/DICTIONARY_NOTICE.md`를 정본으로 사용한다. 정확한 입력 URL·SHA-256·생성 스크립트·재현 절차를 복구했고, 사전 본문은 수정·삭제하지 않았다. 로컬 라이선스 표기와 `LICENSE.GPL-3` 전문·화면 링크를 보완했으며 운영 반영만 승인 대기다.
...
- 9~12단계 로컬 구현·검증은 완료했다.
- `kbo-owner`의 경영 시스템은 실명 데이터와 공개 범위를 바꾸지 않고 구현했다. 일반 공개 방식은 여전히 `D-06`에서 결정해야 한다.
- `life-diary`는 출처 문제가 없는 코드 기반 장면을 사용하며 전용 래스터 이미지 제작은 `D-09`로 보류했다.
- 다음 작업은 보완 목록 `D-01`~`D-12`다. 운영 Firebase 규칙·학생 데이터·유료 에셋 공개 변경은 별도 승인 전 진행 불가다.

## 후속 관리 방식

위 보류 항목과 9~12단계에서 새로 확인된 항목은 `DEFERRED_FOLLOWUPS_AFTER_STEP_12.md`의 `D-01`~`D-12`로 관리한다. 그 문서를 다음 작업 정본으로 삼고, 이 문서의 공개 금지선과 승인 게이트는 그대로 유지한다.
