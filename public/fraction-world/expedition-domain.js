(function(root){
 ...
 function create(weapon,difficulty,mode='short',look='dawn',seed=Math.floor(Math.random()*0xffffffff)){
  const s=D.create(weapon,difficulty);Object.assign(s,{balance:2,expedition:mode==='long'?'long':'short',look,seed,weaponLevel:0,branch:null,mod:null,tech:null,maps:[],visited:[],node:null,bosses:[],regionFights:[0,0,0,0],runes:0,quizAnswered:0,correct:0,hiddenUsed:[],keys:0,elites:[0,0,0,0],hiddenOpen:false});
 ...
 const weaponCap=s=>s.balance===2?[2,3,5,5][region(s)]:5;
 function upgradeWeapon(s,branch){const lv=s.weaponLevel||0,cost=[15,25,40,60,85][lv];if(lv>=weaponCap(s)||!['route','room'].includes(s.phase)||s.crystals<cost||lv>=2&&s.node?.type!=='forge')return false;if(lv===2&&!['a','b'].includes(branch))return false;s.crystals-=cost;s.weaponLevel=lv+1;if(lv===2)s.branch=branch;return true;}
