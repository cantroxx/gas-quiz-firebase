(function(root){
  ...
  const A=typeof module!=='undefined'&&module.exports?require('./adventure-rules.js'):root.FWAdventure;
  ...
  function stats(s){
    ...
    const lv=s.weaponLevel||0;result.damage*=1+lv*.12;result.interval/=1+lv*.04;result.skillDamage*=1+lv*.1;result.reach=(w.reach||0)*(1+lv*.07);if(s.branch==='b'&&!w.reach&&w.id!=='book')result.count++;if(s.branch==='a')result.damage*=1.15;if(lv===5){result.skillCooldown*=.85;result.pierce++;}if(s.mod==='focus'){result.damage*=1.2;result.reach*=1.2;}if(modern(s)){result.interval=Math.max(w.interval/1.65,result.interval);result.armor=Math.max(.5,result.armor);result.skillCooldown=Math.max(6,result.skillCooldown);result.effectiveCount=w.count+(result.count-w.count)*.45;}else result.effectiveCount=result.count;if(s.controlVersion>=1){result.attackInterval=Math.max(.32,result.interval*1.35);result.autoInterval=Math.max(3.5,result.interval*7);}result.attackInterval=result.attackInterval||result.interval;if(s.rulesVersion===2){result.attackInterval=Math.max(.32,result.interval);result.autoInterval=Math.max(1.8,result.attackInterval*3);result.armor*=1-(s.tier==='beginner'?Math.min(4,s.coinFailures||0)*.05:0);}const boost=s.rulesVersion===2?1+A.bonus(s):s.playMode==='coin'?1+Math.min(20,s.coinFailures||0)*.25:1;result.damage*=boost;result.skillDamage*=boost;return result;
  }
  function roomSpec(s){const depth=s.expedition==='long'?8:6,biome=Math.min(3,Math.floor((s.room-1)/depth));const boss=s.room%depth===0;const elite=s.route==='elite';
    if(s.rulesVersion===2){const t=A.get(s),base=roomSpec({...s,rulesVersion:undefined,difficulty:'hard',expedition:'short'});return {...base,hpScale:base.hpScale*t.hp,damageScale:base.damageScale*t.damage,speedScale:base.speedScale*t.speed,maxAlive:t.cap[biome],packSize:Math.min(base.packSize,s.tier==='beginner'?2:3),warning:t.warning,spawnInterval:base.spawnInterval*(s.tier==='beginner'?1.25:1)};}
  ...
  function completeRoom(s,rng=Math.random){if(s.phase!=='combat')return false;s.best=Math.max(s.best,s.room);s.crystals+=roomSpec(s).reward+luckBonus(s);s.hp=Math.min(stats(s).maxHp,s.hp+Math.min(modern(s)?12:Infinity,(s.relics.includes('heal')?10:0)+(s.relics.includes('root')?6:0)));s.offers=offers(s,rng);if(s.expedition&&roomSpec(s).boss&&!s.bosses.includes(roomSpec(s).biome))s.bosses.push(roomSpec(s).biome);s.coinFailures=0;s.retryPending=false;delete s.roomStartKills;s.phase='reward';return true;}
