(function(root){
  ...
  function upgrade(s,id){
    if(s.phase!=='route'||!s.relics.includes(id)||level(s,id)>=3||s.crystals<upgradeCost(s,id))return false;
  ...
  function stats(s){
    ...
    const maxHp=110+power.earth*8+(has('heart')?25:0)+(has('icewall')?15:0)+(tags.earth>=3?20:0);
    return {damage:w.damage*(1+power.fire*.06)*(1+(has('ember')?.22:0)+(has('meteor')?.35:0)+(has('crown')?.45:0)+(has('fury')&&s.hp<maxHp/2?.4:0)+(tags.fire>=3?.2:0))*(has('twin')?.88:1),
  ...
  function roomSpec(s){const biome=Math.min(3,Math.floor((s.room-1)/6));const boss=s.room%6===0;const elite=s.route==='elite';
    const basic=s.difficulty!=='expert';return {biome,boss,elite,speedScale:basic?.85:1,spawnInterval:basic?.75:.55,count:boss?1:Math.ceil((10+biome*3+(s.room%6)+(elite?3:0))*(basic?.8:1)),hpScale:(basic?.9:1)*(1+biome*.38+s.room*.045+(elite?.3:0)),damageScale:(basic?.85:1)*(s.difficulty==='expert'?1.35:1)*(1+biome*.13),reward:boss?45:elite?30:18};}
