(function(){
 ...
 function effect(c,f){const t=1-f.life/f.max;c.save();c.globalAlpha=Math.min(1,f.life*5);const color=f.color||'#fff';
  ...
  else if(f.kind==='ghost'){c.globalAlpha=f.life/f.max*.45;if(f.look){const row=FWContent.looks.find(l=>l.id===f.look)?.row||0;sprite(c,'explorers-v4',4,6,row*4+3,f.x-30,f.y-49,60,80);}else sprite(c,'hero-motion-v2',4,2,6,f.x-30,f.y-49,60,80);}
