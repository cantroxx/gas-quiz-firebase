(function(root){
  ...
  function applyDay(s,plan){
    ...
    for(const id of plan){
      ...
      let fatigued=false;
      s.members.forEach(m=>{
        const gain=m.energy<25?.45:m.energy<45?.75:1;if(gain<1&&a.energy<0)fatigued=true;
    ...
    s.plan=plan.slice();s.last={day:s.day,notes};
  ...
  function perform(s){
    ...
    s.history.push(result);s.cash+=result.passed?c.reward:Math.floor(c.reward*.3);s.fans+=result.passed?25:8;
  ...
  const api={create,average,upcoming,requirements,forecast,validatePlan,applyDay,chooseEvent,perform,advance,buy,selectSong};
