...
async function main(){const browser=await chromium.launch({executablePath:'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',headless:true});try{
 ...
 await page.locator('#combat-lobby:visible, #home-button:visible').click();await page.click('#enter-studio');await page.click('#studio-start');assert.equal(await page.locator('.wardrobe-portrait').count(),3);await page.click('[data-activity="dance"]');assert.equal(await page.locator('.stage').getAttribute('data-practice'),'dance');
