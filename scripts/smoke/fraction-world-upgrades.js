...
async function main(){const browser=await chromium.launch({executablePath:'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',headless:true});try{
 const page=await browser.newPage({viewport:{width:1024,height:768},hasTouch:true}),errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('http://127.0.0.1:5478/fraction-world/');await page.click('#enter-tower');assert.match(await page.locator('[data-diff="hard"]').innerText(),/기본 난이도/);assert.match(await page.locator('[data-diff="expert"]').innerText(),/심화 난이도/);await page.click('#tower-start');await page.evaluate(()=>{FWStore.get().tower=FWTowerDomain.create('wand');FWTower.enter();});
 ...
 await page.evaluate(()=>{const s=FWStore.get().tower;s.phase='reward';s.offers=['fury'];FWStore.save();FWTower.enter();});assert.match(await page.locator('[data-preview="fury"]').innerText(),/불꽃 조합 완성/);assert.match(await page.locator('[data-preview="fury"]').innerText(),/공격력/);
 await page.evaluate(()=>{const s=FWStore.get().tower;s.relics=['ember','crit','fury','heart','frost','storm','nova','clock'];s.offers=['boots'];FWTower.enter();});await page.locator('[data-replace="ember"]').tap();assert.match(await page.locator('[data-preview="boots"]').innerText(),/불꽃 조합 풀림/);
 await page.locator('[data-relic="boots"]').tap();await page.click('#confirm-reward');assert.equal(await page.evaluate(()=>FWStore.get().tower.room),2);await page.click('[data-route="normal"]');assert.ok(await page.locator('#arena').count());await page.locator('#combat-lobby:visible, #home-button:visible').click();
