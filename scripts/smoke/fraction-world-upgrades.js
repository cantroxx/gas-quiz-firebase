...
async function main(){const browser=await chromium.launch({executablePath:'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',headless:true});try{
 ...
 await page.evaluate(()=>{const s=FWStore.get().tower;s.phase='reward';s.offers=['fury'];FWStore.save();FWTower.enter();});assert.match(await page.locator('[data-preview="fury"]').innerText(),/불꽃 조합 활성/);assert.match(await page.locator('[data-preview="fury"]').innerText(),/공격력/);
