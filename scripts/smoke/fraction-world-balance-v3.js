...
async function main(){const browser=await chromium.launch({executablePath:'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',headless:true});try{
 ...
 await page.addInitScript(()=>document.addEventListener('DOMContentLoaded',()=>{const create=FWExpedition.create;FWExpedition.create=(...args)=>({...create(...args),balance:3});}));await page.goto(process.env.FRACTION_WORLD_URL||'http://127.0.0.1:5478/fraction-world/');await page.clock.install();await page.click('#enter-tower');await page.click('#tower-start');
