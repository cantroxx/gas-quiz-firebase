...
async function main(){
 ...
 for(const room of [6,12,18,24]){
  ...
  await page.click('#pause-game');await page.screenshot({path:`/tmp/fraction-world-boss-${room}.png`});await page.locator('#combat-lobby:visible, #home-button:visible').click();
 }
 await fixture(3,'charger');await page.waitForTimeout(6500);await page.click('#pause-game');assert.ok(await page.evaluate(()=>FWStore.get().tower.combat.enemies[0].attacks>=3));await page.locator('#combat-lobby:visible, #home-button:visible').click();
 ...
 await page.locator('[data-relic]').first().click();await page.click('#confirm-reward');await solve();assert.equal(await page.evaluate(()=>FWStore.get().tower.room),2);assert.equal(await page.evaluate(()=>FWStore.get().collection.length),1);
 // The final reward commits completion once, including after a reload.
 await fixture(24,'boss',1,0);await page.waitForSelector('[data-relic]',{timeout:10000});await page.click('#skip-reward');await page.click('#confirm-reward');await solve();assert.equal(await page.evaluate(()=>FWStore.get().tower.cleared),true);assert.equal(await page.evaluate(()=>FWStore.get().records.filter(r=>r.mode==='tower').length),1);
 ...
 await fixture(1,'chaser',1000,999);await page.locator('#combat-lobby:visible, #home-button:visible').click();await page.evaluate(()=>{const s=FWStore.get().tower;s.hp=1;s.combat.p.inv=0;s.combat.enemies[0].x=s.combat.p.x;s.combat.enemies[0].y=s.combat.p.y;FWStore.save();});await page.click('#enter-tower');await page.waitForSelector('#tower-again');assert.equal(await page.evaluate(()=>FWStore.get().tower.cleared),false);
