...
* 2026-06-16 기준 마지막 완료 묶음은 엄격한 클린 아키텍처 라운드 25~36 순차 진행이다.
* 완료된 마지막 리팩터링 커밋은 `83bee17 refactor: move event reward claim wrapper into usecase`이며, 작업 트리는 최종 재개 메모 갱신 후 깨끗한 상태로 종료한다.
* 완료 범위:
  * profile ranking current-member flow를 ranking-usecases로 이동
  * quiz play 선택/키보드 이벤트 flow를 quiz-flow로 이동
  * classroom economy action routing을 classroom-usecases로 통합
  * event reward claim current-member/button wrapper를 event-usecases로 이동
  ...
  * 병렬 작업 계획 문서를 라운드 25~36 완료 상태와 다음 실행 순서 기준으로 갱신
* 마지막 검증:
  * `node tests/application/ranking-usecases.test.js` 통과
  * `node tests/application/quiz-flow.test.js` 통과
  * `node tests/application/classroom-usecases.test.js` 통과
  * `node tests/application/event-usecases.test.js` 통과
  * `npm run check` 통과
...
* 다음 추천 방향은 remaining app-view/bootstrap wrappers, ranking popular filter dependencies, quiz timer/session dependency groups, classroom quest/review wrappers를 `public/index.html` 밖으로 더 이동하는 것이다.
